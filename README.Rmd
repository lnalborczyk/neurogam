...
The goal of `neurogam` is to provide utilities for estimating the onset and offset of effects in M/EEG data. The current version only allows fitting 1D temporal data (e.g., raw M/EEG data or decoding timecourses) but will be extended in the future to support 2D temporal or spatiotemporal data.
...
Below we fit a Bayesian generalised additive multilevel model (BGAMM) to estimate the onset and offset of simulated EEG data.
